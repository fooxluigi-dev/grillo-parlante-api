...
export async function extractTextWithOCRSpace(images: string[], apiKey: string): Promise<OCRSpaceResult> {
  ...
  for (let i = 0; i < images.length; i++) {
    ...
    try {
      ...
      finalB64 = rawData;
      console.log('Sharp fallback:', e instanceof Error ? e.message : 'Unknown error');
    }
    ...
    try {
      ...
      console.error('OCR.space failed:', err instanceof Error ? err.message : 'Unknown error');
    }
  }

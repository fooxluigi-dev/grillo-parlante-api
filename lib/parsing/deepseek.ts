...
import { parsedBookingSchema, type ParsedBooking } from '../schemas/index.js';
...
export async function parseBookingWithDeepSeek(
  extractedText: string,
  apiKey: string
): Promise<DeepSeekParseResult> {
  if (!apiKey) {
    return { data: null, validationErrors: null, error: 'No DeepSeek API key' };
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { data: null, validationErrors: null, error: `DeepSeek ${response.status}: ${errText.slice(0, 150)}` };
    }
    ...
    if (!parsed) {
      return { data: null, validationErrors: null, error: 'Failed to parse DeepSeek response as JSON' };
    }
    ...
    if (!result.success) {
      ...
      return { data: parsed, validationErrors: result.error.flatten().fieldErrors, error: null };
    }

    return { data: result.data, validationErrors: null, error: null };
  } catch (err) {
    return { data: null, validationErrors: null, error: err instanceof Error ? err.message : 'Unknown error' };
  }
}